...
#taking file from command line argument. For example: python3 vt_upload.py path/to/file/wannacry.exe
file_from_hp = sys.argv[1]


#Function to check the ID of the file that has been uploaded
def check_id_of_uploaded_file(id_of_file,hash_of_current_file):
	os.system("sleep 300")
	...
	with open("/output_json/"+hash_of_current_file+".json", "w") as outfile:
...
def upload_file_virustotal(file_from_hp,hash_of_current_file):
	...
	#Reading the file
	files = {'file': (file_from_hp, open(file_from_hp, 'rb'))}
	...
	check_id_of_uploaded_file(id_received,hash_of_current_file)
	#os.system("sleep 20")


#Function to check MD5 hash of the with Virustotal Database. If there is any match, it will give the results
def check_hash_virustotal():

	#Opening the file
	with open(file_from_hp, "rb") as f:
		...
		try:
			...
			if check_error_or_not == "NotFoundError":
				upload_file_virustotal(file_from_hp,malware_md5)
